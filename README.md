...
- `pkg/logx`、`pkg/cache`、`pkg/idgen`、`pkg/database`：基础设施封装；`pkg/logx` 统一封装 zap 初始化、console 终端可读输出、JSONL stdout/文件日志、按日期与大小滚动、oops 错误字段提取和脱敏策略。其中 `pkg/idgen.ID` 是业务 ID 专用类型，底层为 int64、JSON/OpenAPI 为 string；`pkg/database` 提供 SQL 连接池注册与启动 Ping 检查。
