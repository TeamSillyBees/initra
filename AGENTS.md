...
- access JWT 只承载 `userId`、`sessionId`、`sessionVersion` 和标准声明，不保存角色、权限或租户快照；请求时必须通过 `auth.IdentityResolver` 从共享缓存或数据库解析当前有效用户、会话版本、角色和超级管理员状态。全部退出、修改密码或禁用账号必须递增 `sys_user.session_version`。
