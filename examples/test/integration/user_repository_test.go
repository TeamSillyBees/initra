...
func TestUserRepositoryCreateUsesEntHooks(t *testing.T) {
	...
	require.Equal(t, idgen.New(9001), record.CreatedBy)
	require.Equal(t, idgen.New(9001), record.UpdatedBy)
...
func TestUserRepositoryFindByID(t *testing.T) {
	...
	record, err := repo.FindByID(context.Background(), idgen.New(1001))

	require.NoError(t, err)
	require.Equal(t, idgen.New(1001), record.ID)
...
func TestUserRepositoryDeleteSoftDeletesUserAndRoles(t *testing.T) {
	...
	err := repo.Delete(context.Background(), idgen.New(1001), idgen.New(9001))
