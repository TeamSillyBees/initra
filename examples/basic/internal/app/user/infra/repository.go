...
import (
	...
	"github.com/teamsillybees/initra/examples/basic/internal/app/user/domain"
...
// idGenerator 定义仓储层写入 sys_user_role 时需要的关系主键生成能力。
// 之所以在仓储层显式依赖生成器，是因为 sys_user_role 也采用雪花 ID，而不是数据库自增。
type idGenerator interface {
	NextID() int64
}

// Repository 使用 Jet SQL Builder 实现 user 模块仓储。
// 该实现以 sys_user 为主表，并通过 sys_user_role / sys_role 维护角色关联。
...
// List 分页查询用户列表。
func (r *Repository) List(ctx context.Context, input domain.ListUsersInput) ([]*domain.User, int64, error) {
	page := input.Page
	if page <= 0 {
		page = 1
	}
...
// rowScanner 抽象 *sql.Rows 和其他可扫描对象，方便复用扫描逻辑。
type rowScanner interface {
	Scan(dest ...any) error
}

// userRecord 是数据库行到领域 User 之间的中间结构，负责承接 nullable 字段。
...
// selectBase 定义用户查询的基础字段投影，保证详情和列表字段保持一致。
...
// queryOne 执行单条用户查询，并补齐用户角色编码集合。
func (r *Repository) queryOne(ctx context.Context, stmt SelectStatement) (*domain.User, error) {
	executor := platformdb.ExecutorFromContext(ctx, r.db)
	query, args := stmt.Sql()
...
// scanUser 将当前行扫描为领域 User，供列表查询批量复用。
...
// toDomain 将数据库记录转换为领域实体，并把 nullable 字段转换为零值或指针。
...
// resolveRoleIDs 将角色编码解析为角色 ID，并校验所有编码都存在且已启用。
...
// replaceUserRoles 以“先删后插”的方式重建用户角色关系，调用方必须位于事务中。
...
// loadRoleCodesByUserIDs 批量查询用户角色编码，避免列表接口出现 N+1 查询。
...
// normalizeCodes 清理角色编码入参中的空白和重复值。
...
// stringExpressions 将字符串切片转换为 Jet IN 条件所需的表达式切片。
...
// int64Expressions 将 int64 切片转换为 Jet IN 条件所需的表达式切片。
