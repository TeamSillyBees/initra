...
// The SysDictCollectionFunc type is an adapter to allow the use of ordinary
// function as SysDictCollection mutator.
type SysDictCollectionFunc func(context.Context, *ent.SysDictCollectionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SysDictCollectionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SysDictCollectionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SysDictCollectionMutation", m)
}
