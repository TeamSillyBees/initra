...
// Create 创建用户。
func (s *Service) Create(ctx context.Context, body CreateUserBody, operatorID idgen.ID) (UserVO, error) {
...
// Update 更新用户并清理缓存。
func (s *Service) Update(ctx context.Context, id idgen.ID, body UpdateUserBody, operatorID idgen.ID) (UserVO, error) {
	user, err := s.findByID(ctx, id)
	if err != nil {
		return UserVO{}, err
	}
...
func (s *Service) Delete(ctx context.Context, id idgen.ID, operatorID idgen.ID) error {
	if err := s.deleteEnt(ctx, id, operatorID); err != nil {
		return err
	}
...
func (s *Service) deleteEnt(ctx context.Context, id idgen.ID, operatorID idgen.ID) error {
	ctx = entx.WithOperatorID(ctx, operatorID)
	return data.WithinTx(ctx, s.client, func(txCtx context.Context, txClient *appent.Client) error {
