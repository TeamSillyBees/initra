...
func (r *Repository) FindByID(ctx context.Context, id idgen.ID) (*Identity, error) {
	record, err := r.client.SysUser.Query().
		Where(
			sysuser.ID(id),
			sysuser.DeletedAtIsNil(),
...
func (r *Repository) loadRoleCodes(ctx context.Context, userID idgen.ID) ([]string, error) {
	var rows []struct {
