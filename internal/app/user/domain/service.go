...
func (s *Service) Get(ctx context.Context, id int64) (*User, error) {
	...
	if user == nil {
		return nil, apperrors.New(apperrors.CodeUserNotFound, "user not found", apperrors.WithDetail("user_id", id))
	}
...
func (s *Service) Update(ctx context.Context, input UpdateUserInput) (*User, error) {
	...
	if user == nil {
		return nil, apperrors.New(apperrors.CodeUserNotFound, "user not found", apperrors.WithDetail("user_id", input.ID))
	}
