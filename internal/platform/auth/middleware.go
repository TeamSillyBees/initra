...
func RequestContextMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		ctx := sharedtypes.WithRequestID(c.Request.Context(), requestID)
		ctx = sharedtypes.WithTraceID(ctx, traceID)
	...
	return func(c *gin.Context) {
		...
		if logger != nil && c.Writer.Status() >= http.StatusInternalServerError {
			logger.Warn("request completed with server error",
				zap.String("trace_id", sharedtypes.TraceIDFromContext(c.Request.Context())),
...
func RequestLoggerMiddleware(logger *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		traceID := sharedtypes.TraceIDFromContext(c.Request.Context())
		requestID := sharedtypes.RequestIDFromContext(c.Request.Context())
		fields := []zap.Field{
